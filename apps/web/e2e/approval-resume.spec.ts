...
test("approval input resumes durable work", async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole("button", { name: "수정 후 전송" })).toBeVisible();
  await captureScreenshot(page, testInfo, "20-approval-input-request");

  await page.getByRole("button", { name: "수정 후 전송" }).click();
  const answer = page.getByRole("textbox", { name: "답변" });
  await answer.fill("ask me which city to use again");
  await expect(page.getByRole("button", { name: "답변 전송" })).toBeEnabled();
  ...
  await expect(
    page.getByText("답변 완료: ask me which city to use again", { exact: true }),
  ).toBeVisible();
  ...
  await expect(page.getByText("답변 완료: Paris", { exact: true })).toBeVisible();
  ...
  await expect(page.getByText("더 이상 유효하지 않음", { exact: true })).toBeVisible();
