...
async function call(name, path, body) {
  ...
  results.push({
    ...
    error: response.ok
      ? undefined
      : (data?.error?.message ?? data?.message ?? String(data).slice(0, 180)),
  });
}

await call("text-korean", "/chat/completions", {
  model,
  messages: [{ role: "user", content: "한국어로 '브라우저 작업 준비 완료'라고만 답해." }],
  max_tokens: 30,
  temperature: 0,
});
await call("stream", "/chat/completions", {
  model,
  stream: true,
  messages: [{ role: "user", content: "한 단어로 준비 상태를 답해." }],
  max_tokens: 20,
  temperature: 0,
});
await call("json", "/chat/completions", {
  model,
  response_format: { type: "json_object" },
  messages: [{ role: "user", content: 'JSON 객체로 {"ready":true}만 반환해.' }],
  max_tokens: 30,
  temperature: 0,
});
await call("tool-calling", "/chat/completions", {
  model,
  messages: [
    { role: "user", content: "example.com의 페이지 제목을 얻어야 한다. 제공된 도구를 호출해." },
  ],
  tools: [
    {
      type: "function",
      function: {
        ...
        description: "Return a web page title.",
        parameters: {
          type: "object",
